...
export async function feDummy(entorno: Entorno): Promise<{
  AppServer: string;
  DbServer: string;
  AuthServer: string;
}> {
  return llamarWSFE(entorno, 'FEDummy');
}
...
export async function puntosDeVenta(entorno: Entorno, ta: TicketAcceso, cuit: number) {
